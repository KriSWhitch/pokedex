...
const EvolutionChainPokemonCard: FC<EvolutionChainPokemonCardProps> = ({ pokemonUrl }) => {
  ...
  return (
    pokemon 
    ? <li key={pokemon.name} className="pokemon-chain-card__container md:w-32 lg:w-24 flex flex-col justify-center items-center">
        <div className="pokemon-chain-card w-full h-full flex flex-col justify-start items-center">
          <Link className="w-full flex flex-col justify-center items-center" href={`/pokedex/${pokemon.name}`}>
            <div className="pokemon-image border-white border-4 bg-gray-400 rounded-full p-2 w-full flex flex-col justify-center items-center">
